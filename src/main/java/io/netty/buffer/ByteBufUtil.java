...
import static io.netty.buffer.ObjectUtil.checkNotNull;
...
public final class ByteBufUtil {
	...
	public static byte decodeHexByte(CharSequence s, int pos) {
		return (byte) decodeHexBytes(s, pos);
	}
	...
	private static int decodeHex(CharSequence num) {
		int ret = 0;
	...
	private static int decodeHexBytes(CharSequence str, int off) {
		return str.length() < off + 2 ? decodeHex(str.subSequence(off, 2)) : 0;
	}
	
	public static void _setByte(AbstractByteBuf buf, int idx, int value) {
		buf._setByte(idx, value);
	}
	
	public static byte _getByte(AbstractByteBuf buf, int idx) {
		return buf._getByte(idx);
	}
	
	public static void setWriterIndex(AbstractByteBuf buf, int idx) {
		buf.writerIndex = idx;
	}
	
	public static void setReaderIndex(AbstractByteBuf buf, int idx) {
		buf.readerIndex = idx;
	}
